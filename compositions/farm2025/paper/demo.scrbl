...
implementation uses MusicXML@~cite[musicxml-2001], as an intermediary between
the two systems.
...
from the score.  Next, we apply a rewriter called @code{instant->interval}, which
...
attached using @code{@"@"}.  In order to get the names correct, we apply a
...
After applying this rewrite, our sections now have their respective names
attached to their respective notations.

Having finished normalization, the final step is to translate the names in the
Tonart score into Racket bindings, so they can be used in the Racket module system. 
This is done using a Tonart @emph{realizer}.  Tonart realizers
transform Tonart into Racket.  We will use the @code{namespace-define-realizer},
which will transform our Tonart score with named sections into a series of Racket
definitions, one definition per section.  These definitions are combined with the
module system statements from the score editor score, completing the
translation.
...
language, and the showcase will be a short improvised lecture-performance, achieved by
...
for accessibility, it still leaves much to be desired vis-à-vis interoperation
...
each other.  Finally, having an intermediary form like MusicXML can be
inconvenient.@musicxml-note{In the current implementation, many errors are
actually reported with source locations coming from @emph{inside the MusicXML}.
This is somewhat impressive, but misses the mark from a usability perspective.}
...
Output-Directed editor for SVGs, allowing both graphical and textual editing.
Edits to the graphic reflect immediately in the graphic's domain-specific source
code, and edits to the source code immediately reflect on the graphic.  It is
hard to imagine integrating this in an existing score editor.  Nonetheless, an
Output-Directed music editor would minimize intermediate representations,
maximize feedback, and provide good graphical error messages.

@linebreak[]
In lieu of the development of a bespoke extensible score editor, the embedding 
presented in this demo hopefully strikes a good balance between meeting
composers where they are at, and retaining the valuable abstractions music
languages provide.
